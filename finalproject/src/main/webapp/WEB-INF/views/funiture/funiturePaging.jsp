...
%>
   <a href="<%=url%>?<%=str%>mid=<%=request.getParameter("mid")%>&curPage=1">◁◁</a>&nbsp;&nbsp;&nbsp;
   <a href="<%=url%>?<%=str%>mid=<%=request.getParameter("mid")%>&curPage=<%=linkPage%>">◀</a>&nbsp;&nbsp;&nbsp;
<%
   } else {
%>
   ◁◁&nbsp;&nbsp;&nbsp;◀&nbsp;&nbsp;&nbsp;
...
<%
         ...
         //[2][3][4][5]
%>
   [<a href="<%=url%>?<%=str%>mid=<%=request.getParameter("mid")%>&curPage=<%=linkPage%>"><%=linkPage%></a>]&nbsp;
<%
      }
      linkPage++;
      loopCount--;
...
%>
   <a href="<%=url%>?<%=str%>mid=<%=request.getParameter("mid")%>&curPage=<%=linkPage%>">▶</a>&nbsp;&nbsp;&nbsp;
   <a href="<%=url%>?<%=str%>mid=<%=request.getParameter("mid")%>&curPage=<%=pageCount%>">▷▷</a>&nbsp;&nbsp;&nbsp;
<%
   } else {
      System.out.println("다음그룹이 있는 경우 linkPage >>> : " + linkPage);
      System.out.println("다음그룹이 있는 경우 pageCount >>> : " +pageCount);
